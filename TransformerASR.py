...
class TransformerASR(TransformerInterface):
    ...
    def forward_mix(self, src, tgt_current, tgt_previous, wav_len=None, pad_idx=0):
        # reshpae the src vector to [Batch, Time, Fea] is a 4d vector is given
        if src.ndim == 4:
            bz, t, ch1, ch2 = src.shape
            src = src.reshape(bz, t, ch1 * ch2)
        ...
        (
            src_key_padding_mask,
            tgt_current_key_padding_mask,
            src_mask,
            tgt_current_mask,
        ) = self.make_masks(src, tgt_current, wav_len, pad_idx=pad_idx)

        tgt_previous_mask = get_lookahead_mask(tgt_previous)
        tgt_previous_key_padding_mask = get_key_padding_mask(tgt_previous, pad_idx=pad_idx)
        ...
        decoder_out_current = self.forward_decoder( 
            src=src,
            tgt=tgt_current,
            src_mask=src_mask, 
            tgt_mask=tgt_current_mask,
            src_key_padding_mask=src_key_padding_mask,
            tgt_key_padding_mask=tgt_current_key_padding_mask,
            encoder_out=encoder_out
        )
        decoder_out_previous = self.forward_decoder( 
            src=src,
            tgt=tgt_previous,
            src_mask=src_mask, 
            tgt_mask=tgt_previous_mask,
            src_key_padding_mask=src_key_padding_mask,
            tgt_key_padding_mask=tgt_previous_key_padding_mask,
            encoder_out=encoder_out
        )

        return encoder_out, decoder_out_current, decoder_out_previous
